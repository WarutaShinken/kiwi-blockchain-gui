...
import { AlertDialog } from '@kiwi/core';
import {
  Typography,
  Button,
  Box,
...
import { kiwi_to_mojo } from '../../../util/kiwi';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      kiwiper_input.value === '' ||
      Number(kiwiper_input.value) === 0 ||
      !Number(kiwiper_input.value) ||
      isNaN(Number(kiwiper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const kiwiper = kiwi_to_mojo(kiwiper_input.value);
    const kiwiper_value = Number.parseInt(Number(kiwiper));
    ...
    // var fee = kiwi_to_mojo(fee_input.value);
