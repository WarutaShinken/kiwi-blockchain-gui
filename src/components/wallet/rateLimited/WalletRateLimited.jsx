...
import { AlertDialog, Card, Flex } from '@kiwi/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const kiwiper_input = ip_parsed.limit;
    ...
    const kiwiper_value = Number.parseInt(Number(kiwiper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (kiwi per interval):{' '}
                  {mojo_to_kiwi_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = kiwi_to_mojo(amount_input.value);
    const fee = kiwi_to_mojo(fee_input.value);
