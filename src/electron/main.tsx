...
import kiwiEnvironment from '../util/kiwiEnvironment';
import kiwiConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!kiwiEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    const createWindow = () => {
      ...
      if (!kiwiConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !kiwiConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (kiwiConfig.manageDaemonLifetime()) {
        kiwiEnvironment.startKiwiDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Kiwi Blockchain Wiki' }),
            click: () => {
              openExternal(
    ...
    if (process.platform === 'darwin') {
      // Kiwi Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Kiwi' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Kiwi Blockchain' }),
            click: () => {
              openAbout();
            },
          },
