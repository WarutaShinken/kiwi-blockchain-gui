...
const getKiwiVersion = () => {
  let version = null;
  const exePath = getExecutablePath('kiwi');
  // first see if we can get a kiwi exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or kiwi is on the path
    try {
      version = child_process
